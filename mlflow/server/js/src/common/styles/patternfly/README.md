...
|---|---|
| `pf-shell-overrides.scss` | Entry point — imports all partials below |
| `pf-shell-overrides/_scope-and-base-controls.scss` | Buttons, inputs, radio groups, alerts, required field markers |
| `pf-shell-overrides/_dropdowns-selects-and-comboboxes.scss` | Select, DialogCombobox, DropdownMenu, TypeaheadCombobox, virtual lists |
| `pf-shell-overrides/_modals-and-popovers.scss` | Modals, popovers, notifications/toasts |
| `pf-shell-overrides/_layout-navigation-and-tables.scss` | Sidebar, navigation, run tables, comparison tables, sort icons |
| `pf-shell-overrides/_trees-checkboxes-code-and-dark-mode.scss` | Tree view, checkboxes, code blocks, Markdown editor, Plotly dark mode |
| `pf-shell-overrides/_icons.scss` | PatternFly SVG icon definitions (mask-image data URLs) |
| `pf-shell-overrides/_federated-link-guards.scss` | Disables out-of-scope navigation links in federated/embedded mode |
| `patternflyStyles/` | TypeScript files that read `@patternfly/react-tokens` and expose them as typed values |
| `patternflyTokenTranslation.ts` | Assembles the full Emotion theme from all patternflyStyles files |
| `utils.ts` | SCSS helper mixins (floating surface, icon masking) |
...
Supporting files committed in `scripts/`:

| File | Purpose |
|---|---|
| `audit-css-overrides.sh` | Selector existence check script |
| `audit-rebase.sh` | Post-rebase impact report script |
| `css-overrides-baseline.txt` | Known-missing selectors (warn only in CI) |
| `css-overrides-verified-versions.txt` | Last-verified package versions for drift detection |
