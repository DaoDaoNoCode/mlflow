...
import { SunIcon, MoonIcon, CodeIcon, CopyIcon } from '@databricks/design-system';
...
// ---------------------------------------------------------------------------
// Import normalization: ensures imported Puck JSON has valid prop values.
// Handles raw pixel values → token mapping, theme.spacing refs, and invalid enums.
// ---------------------------------------------------------------------------

const SPACING_PX_TO_TOKEN: [number, string][] = [
  [0, 'none'],
  [4, 'xs'],
  [8, 'sm'],
  [16, 'md'],
...
function pxToToken(value: unknown): string {
  if (value === undefined || value === null || value === 0 || value === '0') return 'none';
  if (typeof value === 'string') {
    if (['none', 'xs', 'sm', 'md', 'lg'].includes(value)) return value;
    const tokenMatch = value.match(/theme\.spacing\.(\w+)/);
    if (tokenMatch) return tokenMatch[1];
    const num = parseInt(value, 10);
    if (isNaN(num)) return 'sm';
    return closestToken(num);
  }
  if (typeof value === 'number') return closestToken(value);
  return 'sm';
}

function closestToken(px: number): string {
  for (const [threshold, token] of SPACING_PX_TO_TOKEN) {
    if (px <= threshold + 2) return token;
  }
  return 'lg';
}

const VALID_DISPLAY = new Set(['flex', 'grid', 'block']);
const VALID_FLEX_DIR = new Set(['row', 'column']);
const VALID_FLEX_WRAP = new Set(['nowrap', 'wrap']);
const VALID_JUSTIFY = new Set(['flex-start', 'center', 'flex-end', 'space-between', 'space-around']);
const VALID_ALIGN = new Set(['flex-start', 'center', 'flex-end', 'stretch', 'baseline']);
const VALID_OVERFLOW = new Set(['visible', 'hidden', 'auto']);
const VALID_FLEX = new Set(['none', 'grow', 'auto']);
const VALID_BG = new Set(['none', 'primary', 'secondary']);
const VALID_BORDER = new Set(['none', 'all', 'top', 'bottom', 'decorative']);
const VALID_RADIUS = new Set(['none', 'sm', 'md']);

function normalizeLayoutBoxProps(props: Record<string, any>): Record<string, any> {
  return {
    ...props,
    display: VALID_DISPLAY.has(props.display) ? props.display : 'flex',
    flexDirection: VALID_FLEX_DIR.has(props.flexDirection) ? props.flexDirection : 'column',
    flexWrap: VALID_FLEX_WRAP.has(props.flexWrap) ? props.flexWrap : 'nowrap',
    justifyContent: VALID_JUSTIFY.has(props.justifyContent) ? props.justifyContent : 'flex-start',
    alignItems: VALID_ALIGN.has(props.alignItems) ? props.alignItems : 'stretch',
    gridColumns: typeof props.gridColumns === 'string' ? props.gridColumns : '1fr 1fr',
    gridRows: typeof props.gridRows === 'string' ? props.gridRows : 'auto',
    gap: pxToToken(props.gap),
    rowGap: props.rowGap ? pxToToken(props.rowGap) : '',
    columnGap: props.columnGap ? pxToToken(props.columnGap) : '',
    padding: pxToToken(props.padding),
    flex: VALID_FLEX.has(props.flex) ? props.flex : 'none',
    overflow: VALID_OVERFLOW.has(props.overflow) ? props.overflow : 'visible',
    width: typeof props.width === 'string' ? props.width : 'auto',
    minHeight: typeof props.minHeight === 'string' ? props.minHeight : 'auto',
    maxHeight: typeof props.maxHeight === 'string' ? props.maxHeight : 'none',
    height: typeof props.height === 'string' ? props.height : 'auto',
    background: VALID_BG.has(props.background) ? props.background : 'none',
    border: VALID_BORDER.has(props.border) ? props.border : 'none',
    borderRadius: VALID_RADIUS.has(props.borderRadius) ? props.borderRadius : 'none',
  };
}

function normalizeContentItem(item: any): any {
  if (!item || typeof item !== 'object') return item;

  // Normalize LayoutBox props to valid values
  if (item.type === 'LayoutBox' && item.props) {
    item.props = normalizeLayoutBoxProps(item.props);
  }

  // Ensure FlexRow/FlexColumn have defaults for new fields
  if (item.type === 'FlexRow' || item.type === 'FlexColumn') {
    const p = item.props || {};
    if (p.gap !== undefined) p.gap = pxToToken(p.gap);
    if (p.overflow === undefined) p.overflow = 'visible';
    if (p.flex === undefined) p.flex = 'none';
    if (p.minHeight === undefined) p.minHeight = 'auto';
    if (p.maxHeight === undefined) p.maxHeight = 'none';
    if (item.type === 'FlexRow' && p.wrap === undefined) p.wrap = true;
  }

  // Recursively normalize children in slots
  if (item.props) {
    for (const [key, value] of Object.entries(item.props)) {
      if (Array.isArray(value)) {
        item.props[key] = value.map(normalizeContentItem);
      }
    }
  }
...
function normalizeImportedData(data: Data): Data {
  return {
    ...data,
    content: (data.content || []).map(normalizeContentItem),
  };
}
...
const ImportModal: React.FC<{
  ...
  const handleImport = () => {
    try {
      ...
      if (Array.isArray(parsed)) {
        const imported: Snapshot[] = parsed
          ...
          .map((item: any, i: number) => ({
            name: item.name,
            data: normalizeImportedData(item.data),
      ...
      onImport({ type: 'page', data: normalizeImportedData(parsed) });
    } catch {
...
const JsonPreviewModal: React.FC<{
  ...
  const json = useMemo(() => {
    if (!data) return '';
    const value = mode === 'agent' ? transformForAgent(data, puckConfig) : data;
    return JSON.stringify(value, null, 2);
  }, [data, mode]);

  return (
    <Modal
      componentId="page-composer.json-preview-modal"
      visible={visible}
      title="Page JSON"
      onCancel={onCancel}
      size="wide"
    ...
    >
      <div css={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.sm }}>
        <div css={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <SegmentedControlGroup
            componentId="page-composer.json-preview-mode"
            name="json-preview-mode"
            value={mode}
            onChange={(e: any) => setMode(e.target.value)}
          >
            <SegmentedControlButton componentId="page-composer.json-preview-mode-page" value="page">
              Page JSON
            </SegmentedControlButton>
            <SegmentedControlButton componentId="page-composer.json-preview-mode-agent" value="agent">
              AI Agent JSON
            </SegmentedControlButton>
          </SegmentedControlGroup>
          <CopyButton
            componentId="page-composer.json-preview-copy"
            copyText={json}
            showLabel={false}
            icon={<CopyIcon />}
          />
...
export const PageComposer: React.FC<PageComposerProps> = ({ isDarkMode = false, onToggleDarkMode }) => {
    ...
    () => ({
      headerActions: () => (
        <>
          ...
          <Button
            componentId="page-composer.json-preview"
            size="small"
            icon={<CodeIcon />}
            disabled={!hasActiveContent}
      ...
      <div css={{ padding: theme.spacing.md, height: '100vh', display: 'flex', flexDirection: 'column' }}>
        <div css={{ marginBottom: theme.spacing.md, display: 'flex', gap: theme.spacing.sm, alignItems: 'center' }}>
          ...
          <div css={{ marginLeft: 'auto', display: 'flex', gap: theme.spacing.sm, alignItems: 'center' }}>
            ...
            >
              <SegmentedControlButton componentId="page-composer.journey-vp-full" value="0">
                Full
              </SegmentedControlButton>
              <SegmentedControlButton componentId="page-composer.journey-vp-1440" value="1440">
                1440
              </SegmentedControlButton>
              <SegmentedControlButton componentId="page-composer.journey-vp-1280" value="1280">
                1280
              </SegmentedControlButton>
              <SegmentedControlButton componentId="page-composer.journey-vp-768" value="768">
                768
              </SegmentedControlButton>
            </SegmentedControlGroup>
            {onToggleDarkMode && (
    ...
    return (
      <div css={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
        ...
        >
          ...
          >
            <SegmentedControlButton componentId="page-composer.viewport-full" value="0">
              Full
            </SegmentedControlButton>
            <SegmentedControlButton componentId="page-composer.viewport-desktop" value="1440">
              1440
            </SegmentedControlButton>
            <SegmentedControlButton componentId="page-composer.viewport-laptop" value="1280">
              1280
            </SegmentedControlButton>
            <SegmentedControlButton componentId="page-composer.viewport-tablet" value="768">
              768
            </SegmentedControlButton>
          </SegmentedControlGroup>
          {onToggleDarkMode && (
  ...
  return (
    <div css={{ height: '100vh' }}>
      ...
      <JsonPreviewModal visible={jsonPreviewOpen} data={activeData} onCancel={() => setJsonPreviewOpen(false)} />
