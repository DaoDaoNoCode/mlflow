...
const GRID_COLUMN_PRESETS = [
  { label: 'Custom', value: '' },
  { label: 'Label + Content (120px 1fr)', value: '120px 1fr' },
  { label: 'Equal halves (1fr 1fr)', value: '1fr 1fr' },
  { label: 'Equal thirds (1fr 1fr 1fr)', value: '1fr 1fr 1fr' },
  { label: 'Sidebar + Main (250px 1fr)', value: '250px 1fr' },
  { label: 'Fixed + Fluid + Auto (120px 1fr auto)', value: '120px 1fr auto' },
  { label: 'Responsive cards (repeat(auto-fill, minmax(280px, 1fr)))', value: 'repeat(auto-fill, minmax(280px, 1fr))' },
  { label: 'Responsive wide (repeat(auto-fit, minmax(400px, 1fr)))', value: 'repeat(auto-fit, minmax(400px, 1fr))' },
];
...
// Parses raw CSS declarations (e.g. "display: flex; gap: 8px;") into a React style object.
// No eval/Function — safe against arbitrary input, worst case a bad line is silently skipped.
export const parseCssText = (text: string): Record<string, string> => {
  ...
  components: {
    ...
    FlexRow: {
      ...
      fields: {
        ...
        wrap: { type: 'radio', options: [{ label: 'Wrap', value: true }, { label: 'No wrap', value: false }] },
        overflow: {
          type: 'select',
          options: OVERFLOW_OPTIONS,
        },
        flex: { type: 'select', options: FLEX_OPTIONS },
        minHeight: { type: 'text', label: 'Min height (e.g. auto, 200px)' },
        maxHeight: { type: 'text', label: 'Max height (e.g. none, 400px)' },
      },
      defaultProps: {
        gap: 'sm',
      ...
      inline: true,
      render: ({ content: Content, gap, justifyContent, alignItems, wrap, overflow, flex, minHeight, maxHeight, puck }) => {
        const { theme } = useDesignSystemTheme();
        const guideStyles = useEditGuideStyles(puck.isEditing);
        ...
        return (
          <div
            ref={puck.dragRef}
            css={{
              ...(flex !== 'none' && { flex: flex === 'grow' ? 1 : flex }),
    ...
    FlexColumn: {
      ...
      fields: {
        ...
        overflow: { type: 'select', options: OVERFLOW_OPTIONS },
        flex: { type: 'select', options: FLEX_OPTIONS },
        minHeight: { type: 'text', label: 'Min height (e.g. auto, 200px, 100%)' },
        maxHeight: { type: 'text', label: 'Max height (e.g. none, 400px)' },
      },
      defaultProps: {
        gap: 'sm',
      ...
      inline: true,
      render: ({ content: Content, gap, alignItems, justifyContent, overflow, flex, minHeight, maxHeight, puck }) => {
        const { theme } = useDesignSystemTheme();
        const guideStyles = useEditGuideStyles(puck.isEditing);
        return (
          <div
            ...
            css={{
              ...
              justifyContent,
              ...(flex !== 'none' && { flex: flex === 'grow' ? 1 : flex }),
              ...(overflow !== 'visible' && { overflow, minHeight: minHeight === 'auto' ? 0 : undefined }),
              ...(minHeight !== 'auto' && { minHeight }),
              ...(maxHeight !== 'none' && { maxHeight }),
              ...guideStyles,
            }}
    ...
    DuBoisTableSkeleton: {
      ...
      render: ({ lines }) => (
        <TableSkeleton componentId="page-composer.table-skeleton" lines={lines} label="Loading table..." />
      ),
    },
    DuBoisParagraphSkeleton: {
    ...
    LayoutBox: {
      ...
      fields: {
        ...
        gridColumns: { type: 'text', label: 'Grid columns (e.g. 120px 1fr, repeat(auto-fill, minmax(280px, 1fr)))' },
        gridRows: { type: 'text', label: 'Grid rows (e.g. auto, minmax(28px, auto))' },
        gap: { type: 'select', options: SPACING_OPTIONS },
        rowGap: { type: 'select', label: 'Row gap (overrides gap)', options: [{ label: 'Same as gap', value: '' }, ...SPACING_OPTIONS] },
        columnGap: { type: 'select', label: 'Column gap (overrides gap)', options: [{ label: 'Same as gap', value: '' }, ...SPACING_OPTIONS] },
        padding: { type: 'select', options: SPACING_OPTIONS },
        flex: { type: 'select', options: FLEX_OPTIONS },
        overflow: { type: 'select', options: OVERFLOW_OPTIONS },
        width: { type: 'text', label: 'Width (e.g. auto, 100%, 300px)' },
        minHeight: { type: 'text', label: 'Min height (e.g. auto, 200px)' },
        maxHeight: { type: 'text', label: 'Max height (e.g. none, 400px)' },
        height: { type: 'text', label: 'Height (e.g. auto, 100%)' },
        background: { type: 'select', options: BACKGROUND_OPTIONS },
        border: { type: 'select', options: BORDER_OPTIONS },
        borderRadius: { type: 'select', options: BORDER_RADIUS_OPTIONS },
        content: { type: 'slot' },
      },
      defaultProps: {
        display: 'flex',
      ...
      resolveFields: (data: any, { fields }: any) => {
        const isGrid = data.props.display === 'grid';
        const isFlex = data.props.display === 'flex';
        const result = { ...fields };
        if (!isFlex) {
          delete result.flexDirection;
          delete result.flexWrap;
        }
        if (!isGrid) {
          delete result.gridColumns;
          delete result.gridRows;
        }
        if (!isFlex && !isGrid) {
      ...
      inline: true,
      render: (props: any) => {
        const LayoutBoxInner = () => {
          const { theme } = useDesignSystemTheme();
          const guideStyles = useEditGuideStyles(props.puck.isEditing);
          const {
            ...
            background, border, borderRadius, content: Content, puck,
          } = props;

          const gapPx = resolveSpacing(gap, theme);
          const rowGapPx = rowGap ? resolveSpacing(rowGap, theme) : undefined;
          const columnGapPx = columnGap ? resolveSpacing(columnGap, theme) : undefined;
          const paddingPx = resolveSpacing(padding, theme);
          ...
          const borderRadiusMap: Record<string, string | number | undefined> = {
            none: undefined,
            sm: theme.borders.borderRadiusSm,
            md: theme.borders.borderRadiusMd,
          };

          const borderValue = (() => {
            switch (border) {
              case 'all': return `1px solid ${theme.colors.border}`;
              case 'decorative': return `1px solid ${theme.colors.borderDecorative}`;
              default: return undefined;
            }
          })();

          const borderTop = border === 'top' ? `1px solid ${theme.colors.border}` : undefined;
          const borderBottom = border === 'bottom' ? `1px solid ${theme.colors.border}` : undefined;
          ...
          const layoutStyle: Record<string, any> = {
            ...
            ...(display === 'grid' && {
              ...
              gridAutoRows: gridRows !== 'auto' ? gridRows : undefined,
              justifyContent,
              alignItems,
            }),
            gap: gapPx,
            ...(rowGapPx !== undefined && { rowGap: rowGapPx }),
            ...(columnGapPx !== undefined && { columnGap: columnGapPx }),
          };

          const boxCss: Record<string, any> = {
            ...(paddingPx > 0 && { padding: paddingPx }),
            ...(flex !== 'none' && { flex: flex === 'grow' ? 1 : flex }),
            ...(overflow !== 'visible' && { overflow }),
            ...(width !== 'auto' && { width }),
            ...(minHeight !== 'auto' && { minHeight }),
            ...(maxHeight !== 'none' && { maxHeight }),
            ...(height !== 'auto' && { height }),
            ...(bgMap[background] && { backgroundColor: bgMap[background] }),
            ...(borderValue && { border: borderValue }),
            ...(borderTop && { borderTop }),
            ...(borderBottom && { borderBottom }),
            ...(borderRadiusMap[borderRadius] && { borderRadius: borderRadiusMap[borderRadius] }),
            ...guideStyles,
          };

          return (
            <div ref={puck.dragRef} css={boxCss}>
              <Content minEmptyHeight={SLOT_MIN_HEIGHT} style={layoutStyle} />
            </div>
          );
        };
        return <LayoutBoxInner />;
      },
    },
    ListDetailLayout: {
